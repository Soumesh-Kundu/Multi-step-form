<template>
    <div class="flex flex-wrap items-center gap-8">
        <div v-for="(item,index) in options" :key="index" class="relative flex flex-col items-center gap-3 py-5 my-5 font-medium text-gray-400 duration-200 border-2 border-gray-300 rounded-md cursor-pointer px-7 lg:px-10 lg:py-8 md:hover:scale-105" @click="setCard(item.name)" :class="{'bg-white !text-primary-500 !border-primary-500 !scale-110':checkIsThere(item.name)}">

<template >
    <input ref="dropDownButton" class="border-0 border-b-2 outline-none bg-transparent !border-b-gray-500 focus:ring-0 my-12"
        type="text" @focus="() => { dropDown.show() }" @input="()=>{data[field]=''}"  v-model="searchInput" :placeholder="placeHolder" />
    <div ref="dropDownList" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 ">
        <ul v-if="filterOptions.length>0" class="flex flex-col gap-2 py-2 overflow-auto text-gray-700 max-h-48 custom-scrollbar">
